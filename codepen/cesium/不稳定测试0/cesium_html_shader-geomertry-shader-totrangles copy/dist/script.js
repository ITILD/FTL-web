...
// czm_computePosition返回模型坐标中相对于眼睛的位置，同时考虑当前场景模式：3D、2D 或哥伦布视图
// czm_modelViewProjectionRelativeToEye
// 表示 4x4 模型视图投影转换矩阵，用于将模型坐标（相对于眼睛）转换为剪辑坐标。
// 裁剪坐标是顶点着色器输出的坐标系。这与czm_translateRelativeToEye结合使用。gl_Position
// gl_FragColor =vec4(hsv2rgb(vec3(v_color+czm_frameNumber/500.0 ,1.0,1.0)),0.5);\
function getFS() {
  return "varying float v_color;\
          vec3 hsv2rgb(vec3 c) {\
